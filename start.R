...
options(app.distance_cutoff = 3.5)
options(app.mfx = T)
options(app.directional = 'Ignore')
options(app.taxa = 'human', app.ontology = 'DO')
options(app.all_taxa = list(`H. sapiens` = 'human', `M. musculus` = 'mouse', `R. norvegicus` = 'rat', TEST = 'artificial'))
...
options(app.all_options = list(pv = getOption('app.pv'), fc.lower = getOption('app.fc_lower'),
                               ...
                               filterSame = getOption('app.filter'), directional = getOption('app.directional')))

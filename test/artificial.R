...
experiment.data <- mclapply(1:N_EXPERIMENTS, function(experiment) {
  ...
  if(USE_DESEQ) {
    ...
    contrasts.fit(fit, contr) %>% eBayes %>% topTable(sort.by = 'P', number = Inf) %>%
      as.data.table(keep.rownames = T) %>% .[, .(entrez.ID = rn, fc = logFC, adj.pv = adj.P.Val)]
  }
})
...
geneAssociations <- function(N) {
  ...
  de.cat <- sample(DATA.HOLDER$human@experiment.meta[, unique(cf.CatLongUri)], N, T,
                   DATA.HOLDER$human@experiment.meta[, .(sum(!is.na(cf.ValLongUri))), cf.CatLongUri] %>%
                     .[, V1/sum(V1)])
...
exp.assoc <- rbindlist(mclapply(experiment.data, function(experiment) {
  ...
  if(N == 0) {
    return(data.table(cf.CatLongUri = NA,
                      ee.TagLongUri = NA,
                      cf.ValLongUri = NA,
                      cf.BaseLongUri = NA,
                      n.DE = 0,
  ...
  whichGenes <- genes$entrez.ID[sample(1:nrow(genes), sample(1:N, 1, prob = c(0.8, 0.15, 0.05)[1:N]))]
  ...
  if(length(possibleTags) == 0)
    ...
    N <- DATA.HOLDER$human@experiment.meta[cf.ValLongUri %in% possibleTags | cf.BaseLongUri %in% possibleTags,
                                           length(ee.TagLongUri)] %>% min(3)
    message(N)
    if(N == 0) {
      ee.TagLongUri <- NA
    } else {
      ee.TagLongUri <- DATA.HOLDER$human@experiment.meta[cf.ValLongUri %in% possibleTags | cf.BaseLongUri %in% possibleTags,
                                                         sample(ee.TagLongUri, sample(1:N, 1, prob = c(0.8, 0.15, 0.05)[1:N]))] %>%

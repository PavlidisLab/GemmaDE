lapply(sample(1:nrow(DATA.HOLDER$artificial@experiment.meta), 10), function(i) {
...
lm(mvsm^2 ~ zscore, data.to.plot)

data.to.plot[sample(1:nrow(data.to.plot), 100000), ] %>%
  ggplot(aes(mvsm^2, zscore)) + geom_point(alpha = 0.1) +
  geom_abline(slope = 1, intercept = 0, colour = 'red') +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 15)) + scale_y_continuous(expand = c(0, 0), limits = c(0, 15)) +
  theme_cowplot(font_size = 19) +
  ggtitle('Comparison of experiment ranking methods') +
  xlab(expression(M-VSM^2)) + ylab('Weighted Score')
...
data.to.plot %>% melt(measure.vars = c('mvsm', 'zscore')) %>%
  ...
  scale_y_continuous(sec.axis =
                       sec_axis(~10^(-.),
                                ...
                                breaks = c(1, 0.75, 0.5, 0.4, 0.3, 0.2, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0001),
                                labels = c('100%', '75%', '50%', '40%', '30%', '20%', '10%', '5%', '1%', '0.5%', '0.1%', '0.01%'))) +
...
lapply(c('mvsm', 'zscore'), function(method) {
  ...
  tmp %>% merge(DATA.HOLDER$human@experiment.meta[, .(rsc.ID, cf.Baseline, cf.Val)],
    ...
    .[, isProper := grepl('male', cf.Baseline) | grepl('male', cf.Val)] %>%
    .[, method := method]
}) -> human.plots
...
rbindlist(mouse.plots) %>% .[score <= 2, score, .(isProper, method)] %>%
  .[, isProper := ifelse(isProper, 'Yes', 'No')] %>%
  .[, method := ifelse(method == 'mvsm', 'M-VSM', 'Weighted Score')] %>%
  ggplot(aes(isProper, score, color = isProper)) + geom_violin() +
  coord_flip() + stat_summary(fun = median, shape = 3, size = 2) +
  facet_wrap(~method, scales = 'free') + scale_y_continuous(expand = c(0, 0)) +
  theme_cowplot(font_size = 20) + scale_color_discrete(name = 'Male/female contrast') +
...
rbindlist(human.plots) %>% .[, species := 'Human'] %>% .[, .(score, isProper, method, species)] %>% rbind(
  ...
  ggtitle('Male/female contrasts based on KDM5D, XIST and RPS4Y1')
